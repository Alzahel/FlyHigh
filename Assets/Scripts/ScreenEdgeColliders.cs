...
using UnityEngine;
 ...
 {
    [SerializeField] private bool topCollider = true;
    [SerializeField] private bool bottomCollider = true;
    [SerializeField] private bool leftCollider = true;
    [SerializeField] private bool rightCollider =true;
    ...
    //Create our GameObjects and add their Transform components to the Dictionary we created above
        if(topCollider) colliders.Add("Top",new GameObject().transform);
        if(bottomCollider) colliders.Add("Bottom",new GameObject().transform);
        if(rightCollider) colliders.Add("Right",new GameObject().transform);
        if(leftCollider) colliders.Add("Left",new GameObject().transform);
    ...
    //Change positions to align perfectly with outter-edge of screen, adding the world-space values of the screen we generated earlier, and adding/subtracting them with the current camera position, as well as add/subtracting half out objects size so it's not just half way off-screen
       ...
       if(rightCollider) colliders["Right"].position = new Vector3(cameraPos.x + screenSize.x + (colliders["Right"].localScale.x * 0.5f), cameraPos.y, zPosition);
       if(leftCollider) colliders["Left"].position = new Vector3(cameraPos.x - screenSize.x - (colliders["Left"].localScale.x * 0.5f), cameraPos.y, zPosition);
       if(topCollider) colliders["Top"].position = new Vector3(cameraPos.x, cameraPos.y + screenSize.y + (colliders["Top"].localScale.y * 0.5f), zPosition);
       if(bottomCollider) colliders["Bottom"].position = new Vector3(cameraPos.x, cameraPos.y - screenSize.y - (colliders["Bottom"].localScale.y * 0.5f), zPosition);
    }
